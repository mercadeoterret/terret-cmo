...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: '#f5f4ef', colorScheme: 'light' as const }}>
      <aside style={{ width: 220, flexShrink: 0, background: '#f0efe8', borderRight: '2px solid #e0dfd5', display: 'flex', flexDirection: 'column' as const, overflowY: 'auto' as const }}>
        ...
        <nav style={{ flex: 1, padding: 12 }}>
          {NAV.map((item, i) => {
            if ('section' in item) return (
              <div key={i} style={{ fontSize: 10, color: '#9c9a92', textTransform: 'uppercase' as const, letterSpacing: 1, padding: '14px 8px 4px', fontWeight: 700 }}>{item.section}</div>
            )
      ...
      <main style={{ flex: 1, overflowY: 'auto' as const, padding: 28, background: '#f5f4ef', color: '#1a1a18' }}>
        {children}
      </main>
    </div>
  )
